...
struct AnisoHyperDiffusEquations2D{RealT <: Real} <:
       AbstractAnisoHyperDiffusEquations{2, 5}
...
function AnisoHyperDiffusEquations2D(; Kpara = 1.0, Kperp=0.1, Lr = inv(2pi))
    Tr = Lr^2 / max(Kpara, Kperp) # So that one is advecting at speed sqrt(Kappa_para/Tr) ?
    AnisoHyperDiffusEquations2D(promote(Lr, inv(Tr), Kpara, Kperp)...)
end

varnames(::typeof(cons2cons),    ::AnisoHyperDiffusEquations2D) = ("phi", "q1", "q2", "bx", "by")
varnames(::typeof(cons2prim),    ::AnisoHyperDiffusEquations2D) = ("phi", "q1", "q2", "bx", "by")
varnames(::typeof(cons2entropy), ::AnisoHyperDiffusEquations2D) = ("phi", "q1", "q2", "bx", "by")
...
@inline function initial_condition_poisson_nonperiodic(x, t,
    ...
    if iszero(t)
        ...
        q2 = one(T)
        bx = one(T)
        by = one(T) # global field at 45 degrees
    else
    ...
    return SVector(phi, q1, q2, bx, by)
end
...
"""
    source_terms_harmonic(u, x, t, equations::AnisoHyperDiffusEquations2D)
...
@inline function source_terms_harmonic(u, x, t,
    ...
    return SVector(0, du2, du3, 0, 0)
end

"""
    initial_condition_eoc_test_coupled_euler_gravity(x, t, equations::AnisoHyperDiffusEquations2D)
